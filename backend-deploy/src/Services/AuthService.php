...
class AuthService
{
     ...
     */
    public function generateAccessToken(array|object $user): string
    {
        $now = time();

        // Handle both array and object (Eloquent model)
        if (is_object($user)) {
            $user = $user->toArray();
        }
     ...
     */
    public function generateRefreshToken(array|object $user): string
    {
        $now = time();
     ...
     */
    public function generateLoginResponse(array|object $user): array
    {
     ...
     */
    public function isAccountActive(array|object $user): bool
    {
        $isActive = is_array($user) ? $user['is_active'] : $user->is_active;
     ...
     */
    public function isEmailVerified(array|object $user): bool
    {
        $emailVerified = is_array($user) ? $user['email_verified'] : $user->email_verified;

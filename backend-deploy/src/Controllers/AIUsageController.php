...
class AIUsageController
{
    ...
    public function trackUsage(Request $request, Response $response): Response
    {
        try {
            ...
            // Get authenticated user ID (prevents user_id spoofing)
            ...
            // Validate required fields (user_id no longer required from body - we use authenticated user)
            $requiredFields = ['action_type', 'tokens_input', 'tokens_output', 'cost_usd', 'duration_ms'];
            ...
            // Create usage log using authenticated user ID
            $usageId = AIUsageLog::create([
                'user_id' => (int) $authenticatedUserId,

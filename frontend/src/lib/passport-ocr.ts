...
export function findMRZLines(text: string): string[] {
  ...
  const mrzLines = lines
    .filter((line) => {
      ...
      const hasMultipleFillersOrDoubleChevron =
      ...
      return (
        isLongEnough &&
        isNotTooLong &&
        hasMultipleFillersOrDoubleChevron &&
        hasMRZPattern
      );
    })
    .map((line) => {
...
export function parseMRZ(
  ...
  try {
    ...
    return {
      ...
      gender:
        fields.sex === "M" ? "male" : fields.sex === "F" ? "female" : "other",
...
function extractFromReadableText(text: string): PassportData | null {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        line.match(/given\s*names|pr[eé]noms/i) &&
        nextLine &&
        !data.firstName
      ) {
      ...
      // Nationality
      if (
        line.match(/nationality|nationalit[eé]/i) &&
        nextLine &&
        !data.nationality
      ) {
        ...
        if (!found && isoMap[nat]) {
          ...
          data.nationality =
            nat.charAt(0).toUpperCase() + nat.slice(1).toLowerCase();
        }
      }
      ...
      if (line.match(/date\s*of\s*birth|date\s*de\s*naissance/i)) {
        const dateMatch = nextLine.match(
          /(\d{1,2})\s*[A-Z]{3}[A-Z]*[\s\/]*[A-Z]*\s*(\d{2,4})/i,
        );
        if (dateMatch && !data.dateOfBirth) {
          ...
          const month = monthStr
            ? monthMap[monthStr.substring(0, 3)] || "01"
            : "01";
          const fullYear =
            year.length === 2
              ? parseInt(year) <= 30
...
export async function processPassportImage(
  ...
  try {
    ...
    if (mrzLines.length >= 2) {
      ...
      if (passportData) {
        console.log(
          "✅ Passport data extracted successfully via MRZ:",
          passportData,
        );
        return passportData;
      }
      ...
      console.warn(
        `⚠️ Found ${mrzLines.length} MRZ lines (need 2), trying fallback extraction...`,
      );
    }
    ...
    if (fallbackData) {
      console.log(
        "✅ Passport data extracted via fallback method:",
        fallbackData,
      );
      return fallbackData;
    }

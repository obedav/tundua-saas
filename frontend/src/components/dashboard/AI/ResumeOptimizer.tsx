...
export default function ResumeOptimizer() {
  ...
  const downloadAsPDF = () => {
    ...
    const sanitizedResume = DOMPurify.sanitize(optimizedResume.optimizedResume, {
    ...
    element.innerHTML = `
      ...
      <pre style="white-space: pre-wrap; font-family: inherit;">${sanitizedResume}</pre>
    `
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">AI Resume/CV Optimizer</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Target Information */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Target Information</h2>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Current Resume/CV</h2>
            <div className="flex items-center gap-3">
              <span className={`text-sm ${resumeWords < 100 ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`}>
          ...
          <textarea
            ...
            className={`w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 font-mono text-sm dark:bg-gray-700 dark:text-white ${
              resumeWords < 100 ? 'border-red-300' : 'border-gray-300 dark:border-gray-600'
            }`}
...
..."
      ...
      {optimizedResume && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Overall Quality Score</h2>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {optimizedResume.overallScore >= 85 && (
                  <span className="text-green-700 dark:text-green-400 font-semibold">Excellent! Your resume is highly optimized. ✨</span>
                )}
                {optimizedResume.overallScore >= 70 && optimizedResume.overallScore < 85 && (
                  <span className="text-yellow-700 dark:text-yellow-400 font-semibold">Good! A few improvements will make it excellent.</span>
                )}
                {optimizedResume.overallScore >= 60 && optimizedResume.overallScore < 70 && (
                  <span className="text-yellow-700 dark:text-yellow-400 font-semibold">Decent, but needs work in key areas below.</span>
                )}
                {optimizedResume.overallScore < 60 && (
                  <span className="text-red-700 dark:text-red-400 font-semibold">Needs significant improvements - focus on high priority items.</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Key Improvements</h2>
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-gray-900 dark:text-white">{improvement.category}</h3>
                      </div>
                    ...
                    <ul className="space-y-2">
                      {improvement.suggestions.map((suggestion, sIdx) => (
                        <li key={sIdx} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
          ...
          {optimizedResume.missingElements.length > 0 && (
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border-2 border-yellow-300 dark:border-yellow-700 rounded-lg p-6 shadow-md">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-2xl">⚠️</span>
                <h3 className="font-bold text-yellow-900 dark:text-yellow-300 text-lg">Missing Elements</h3>
                <span className="ml-auto px-3 py-1 bg-yellow-200 dark:bg-yellow-900/30 text-yellow-900 dark:text-yellow-300 text-xs font-semibold rounded-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {optimizedResume.missingElements.map((element, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-3 bg-white dark:bg-gray-800 border border-yellow-200 dark:border-yellow-700 rounded-lg p-3 hover:border-yellow-400 transition-all"
                  >
                    <span className="text-yellow-600 dark:text-yellow-400 text-lg mt-0.5">📋</span>
                    <span className="text-sm text-gray-800 dark:text-gray-300 flex-1">{element}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            {showComparison ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Before */}
                <div>
                  <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 bg-red-50 dark:bg-red-900/20 p-2 rounded">
                    Before (Original)
                  </h3>
                  <div className="prose max-w-none">
                    <pre className="whitespace-pre-wrap text-xs text-gray-800 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 p-4 rounded border border-gray-200 dark:border-gray-700 h-96 overflow-y-auto">
                ...
                {/* After */}
                <div>
                  <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 bg-green-50 dark:bg-green-900/20 p-2 rounded">
                    After (Optimized)
                  </h3>
                  <div className="prose max-w-none">
                    <pre className="whitespace-pre-wrap text-xs text-gray-800 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 p-4 rounded border border-green-200 dark:border-green-700 h-96 overflow-y-auto">
              ...
              <div className="prose max-w-none">
                <pre className="whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 p-6 rounded border border-gray-200 dark:border-gray-700">
                  {optimizedResume.optimizedResume}
                </pre>
              </div>
            )}
          ...
          <div className="flex gap-3">
            ...
            <div className="relative download-dropdown">
              ...
              {showDownloadMenu && (
                <div className="absolute bottom-full mb-2 left-0 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-10">
                  <button
                    onClick={() => {
                      downloadAsWord()
                      setShowDownloadMenu(false)
                    }}
                    className="w-full px-4 py-2 text-left hover:bg-blue-50 dark:hover:bg-blue-900/20 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-100 dark:border-gray-700"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 text-left hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-100 dark:border-gray-700"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300"
                  >
            ...
            <button
              onClick={() => setShowComparison(!showComparison)}
              className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-300 rounded-md transition-colors"
            >

...
class ApplicationController
{
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            // Check ownership - allow if user owns the application OR if user is admin/super_admin
            $isOwner = (int)$application->user_id === (int)$userId;
    ...
    public function submit(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            if (!$application->destination_country) {
                $missingFields[] = 'destination_country';
            }
            if (!$application->service_tier_id) {
                $missingFields[] = 'service_tier_id';
            }

            if (!empty($missingFields)) {
                error_log("Application submission validation failed for ID {$id}. Missing fields: " . implode(', ', $missingFields));
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'error' => 'Application is incomplete. Missing: ' . implode(', ', $missingFields),
            ...
            if (!$success) {
                error_log("Application::submitApplication failed for ID {$id}. Check application model error logs.");
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'error' => 'Failed to submit application. Please check server logs for details.'
                ]));
                return $response->withStatus(500)->withHeader('Content-Type', 'application/json');
            }
    ...
    /**
     * Get single application (admin - no ownership check)
    ...
    public function getApplication(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            error_log("Error getting application (admin): " . $e->getMessage());
            $response->getBody()->write(json_encode([
                'success' => false,
                'error' => 'Internal server error'
            ]));
     ...
     */
    public function addAdminNotes(Request $request, Response $response, array $args): Response
    {
        try {
            $id = (int) $args['id'];
            $data = $request->getParsedBody();
            ...
            // Update admin notes using Eloquent
            ...
            error_log("Error adding admin notes: " . $e->getMessage());
            $response->getBody()->write(json_encode([
                'success' => false,
                'error' => 'Internal server error'
            ]));
